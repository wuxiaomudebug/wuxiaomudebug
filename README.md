## Hi there 👋 欢迎来到我的开发者空间！

<!--
**wuxiaomudebug/wuxiaomudebug** 是通往主账号的桥梁，这里存放着我的实验性项目和技术探索记录。
-->

### 🔭 关于我
我是 **WuXiaoMu**（江湖人称「削木」），一名全栈开发者兼开源爱好者。  
📌 这是我的**技术实验账号**，主账号请访问 👉 **[WuXiaoMuer](https://github.com/WuXiaoMuer)**  
🚀 专注领域：现代开发 | 架构 | 开发者工具链

### 🌱 当前动态
- 📦 正在开发 [**EPL**](https://github.com/WuXiaoMuer/EPL) - 一款面向初学者的易语言资料包
- 🧪 实验性项目：~~用Rust重写Node.js核心模块（[查看实验分支](https://github.com/wuxiaomudebug/node-rust-core)）~~
- 📚 学习方向：WebAssembly底层优化 & 分布式系统设计模式 & 开发 & 硬件设计

### 🛠️ 技术栈亮点
![](https://img.shields.io/badge/TypeScript-3178C6?logo=typescript&logoColor=white)
![](https://img.shields.io/badge/Rust-000000?logo=rust&logoColor=white)
![](https://img.shields.io/badge/Kubernetes-326CE5?logo=kubernetes&logoColor=white)
![](https://img.shields.io/badge/GraphQL-E10098?logo=graphql&logoColor=white)
![](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![](https://img.shields.io/badge/Java-007396?logo=java&logoColor=white)
![](https://img.shields.io/badge/C++-00599C?logo=c%2B%2B&logoColor=white)
![](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)
![](https://img.shields.io/badge/易语言-FFC107?logo=data:image/svg+xml;base64,)

### 📌 推荐项目
[![github-contributions](https://github-readme-stats.vercel.app/api/pin/?username=WuXiaoMuer&repo=MuDevTools&theme=vue)](https://github.com/iicsc/github-contributions)

### 📫 找到我
- 主账号邮箱：**wuxiaomuer@gmail.com**  
- 技术博客：[https://wuxiaomu.dev](https://wuxiaomuer.github.io/)  
- X: [@WuXiaoMuer](https://x.com/WuXiaoMuer)

### ⚡ 冷知识
- 曾在48小时内完成从零到部署的项目
- 坚持制作自媒体
- 能用EPL完成99%的开发工作

🌟 **重要提示**：完整项目请访问主仓库 **[WuXiaoMuer](https://github.com/WuXiaoMuer)**，这里主要存放实验性代码和开发日志！
